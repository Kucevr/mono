...
export function FloatingFooter() {
  ...
  return (
    <div
      ref={footerRef}
      className={cn(
        "fixed bottom-8 md:bottom-8 left-1/2 -translate-x-1/2 z-[9999] pointer-events-none w-[95%] sm:w-auto overflow-hidden",
      )}
    >
      <div className="bg-white text-black px-4 md:px-6 py-2 md:py-3 rounded-full shadow-[0_8px_32px_rgba(0,0,0,0.15)] pointer-events-auto flex items-center justify-between sm:justify-start gap-3 sm:gap-6 font-semibold text-xs sm:text-base">
        ...
        <Link to="/" className="flex items-center gap-1.5 md:gap-2 px-1 md:px-2 group whitespace-nowrap">
          {/* Logo dot */}
          <div className="w-5 h-5 md:w-7 md:h-7 shrink-0 rounded-full bg-gradient-to-tr from-[#f953c6] via-[#6B5FFF] to-[#5C90FF] group-hover:scale-110 transition-transform duration-300" />
